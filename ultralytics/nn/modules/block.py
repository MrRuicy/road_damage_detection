...
#     def __init__(self, c1, reduction_ratio=4, heads=8, pool_size=7):
#         super().__init__()
#         self.c1 = c1
#         self.heads = heads
...
#             nn.BatchNorm2d(c1 // (reduction_ratio*2)),
#             nn.ReLU(),
#         )

#         # 多头自注意力
#         self.mhsa = nn.MultiheadAttention(
#             embed_dim=c1 // reduction_ratio, num_heads=heads
#         )
...
#         # 拼接特征
#         fused = torch.cat([local_feat, global_feat.expand_as(local_feat)], dim=1)
#         # 通道压缩
#         compressed = self.channel_compression(fused)
#         # 多头注意力
#         compressed = compressed.view(b, -1, h * w).permute(2, 0, 1)  # (h*w, b, c)
#         attn_output, _ = self.mhsa(compressed, compressed, compressed)
#         attn_output = attn_output.permute(1, 2, 0).view(b, -1, h, w)
#         # 加权融合
#         weights = self.sigmoid(attn_output)
#         return x * weights + x
class MMSA(nn.Module):
    def __init__(self, c1, reduction_ratio=4, heads=8, pool_size=7):
        super().__init__()
        ...
        # 输入通道数应为拼接后的 2*c1
        self.channel_compression = nn.Sequential(
            nn.Conv2d(2 * c1, (2 * c1) // reduction_ratio, 1, bias=False),  # 输入2*c1
            nn.BatchNorm2d((2 * c1) // reduction_ratio),
            nn.ReLU(),
        )

        # 调整多头注意力的embed_dim
        self.embed_dim = (2 * c1) // reduction_ratio
        self.mhsa = nn.MultiheadAttention(
            embed_dim=self.embed_dim, num_heads=heads  # 使用修正后的embed_dim
        )
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        local_feat = self.pool(x)
        global_feat = self.avg_pool(x).expand_as(local_feat)
        fused = torch.cat([local_feat, global_feat], dim=1)  # [B, 2c, H, W]
        compressed = self.channel_compression(fused)  # [B, embed_dim, H, W]
        compressed = compressed.view(b, self.embed_dim, h * w).permute(
            2, 0, 1
        )  # [H*W, B, embed_dim]
        attn_output, _ = self.mhsa(compressed, compressed, compressed)
...
class CARAFE(nn.Module):
    def __init__(self, c1, c2, kernel_size=5, up_factor=2):
        ...
        self.comp = nn.Conv2d(c1, c1 // 4, 1)  # 通道压缩
        self.upsample = nn.Upsample(scale_factor=up_factor, mode="nearest")
        self.pad = (kernel_size - 1) // 2
        self.unfold = nn.Unfold(kernel_size=kernel_size, padding=self.pad)

    def forward(self, x):
        b, c, h, w = x.shape
        # 生成动态核
        compressed = self.comp(x)
        kernel = self.unfold(compressed)  # (b, c*k*k, h*w)
        ...
        out = torch.einsum("bckn,bkln->bcln", kernel, x_unfold)
        return out.view(b, c, h * self.up_factor, w * self.up_factor)
...
class C3k2_DSC(nn.Module):
    """轻量化C3k2模块（使用深度可分离卷积）"""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)  # 标准1x1卷积
        self.cv2 = Conv(c1, c_, 1, 1)
        self.m = nn.Sequential(
            *(Bottleneck_DSC(c_, c_, shortcut, g, k=(3, 3)) for _ in range(n))
        )
        self.cv3 = Conv(2 * c_, c2, 1)  # 输出融合
